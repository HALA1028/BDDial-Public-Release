...
from MEMORY_BANK.memory_bank.build_memory_index import build_memory_index
...
def enter_name(name, memory,local_memory_qa,data_args,update_memory_index=True):
    ...
    #以下为新增HY内容######################################################################################################################
    HY_dir = os.path.join(data_args.memory_basic_dir,data_args.HY_dir)
    HY_index_path = os.path.join(data_args.memory_basic_dir,f'memory_index/HY_index')
    os.makedirs(os.path.dirname(HY_index_path), exist_ok=True)
    if (not os.path.exists(HY_index_path)):
        print(f'Initializing memory index {HY_index_path}...')
        if os.path.exists(HY_index_path):
            shutil.rmtree(HY_index_path)
        HY_index_path, _ = local_memory_qa.init_HY_vector_store(filepath=HY_dir,vs_path=HY_index_path)
    
    user_HY_index = local_memory_qa.load_memory_index(HY_index_path) if HY_index_path else None
    #######################################################################################################################################
    ...
    if name in memory.keys():
        ...
        return msg,user_memory,memory, name,user_memory_index,user_HY_index
    else:
        memory[name] = {}
        ...
        return msg,memory[name],memory,name,user_memory_index,user_HY_index
...
def summarize_memory_event_personality(model,tokenizer,data_args, memory, user_name):
    if isinstance(data_args,gr.State):
        data_args = data_args.value
    if isinstance(memory,gr.State):
        memory = memory.value
    memory_dir = os.path.join(data_args.memory_basic_dir,data_args.memory_file)
    memory = summarize_memory(model,tokenizer,memory_dir,user_name,language=data_args.language)

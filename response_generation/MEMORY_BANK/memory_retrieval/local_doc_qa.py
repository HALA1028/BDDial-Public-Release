...
def get_HY_with_score(docs_with_score):
    ...
    for doc, score in docs_with_score:
        ...
        if score <= similarity_threshold:  # 只保留相似度分数低于阈值的文档
            doc.metadata["score"] = score
            docs.append(doc)
            #selected_questions.add(source_id)
...
class LocalMemoryRetrieval:
    ...
    def search_HY(self,query,vector_store):
        ...
        related_docs,HY_item = get_HY_with_score(related_docs_with_score)
        related_docs = sorted(related_docs, key=lambda x: x.metadata["source"], reverse=False)
